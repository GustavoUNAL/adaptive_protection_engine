...
# ¡¡¡AJUSTA ESTOS VALORES SI USAS OTRA CURVA!!!
CURVE_A = 0.14
CURVE_B = 0.02
MIN_CURRENT_MULTIPLIER = 1.05
MAX_CURRENT_MULTIPLIER = 50
MIN_TIME_PLOT = 0.01
MAX_TIME_PLOT = 20 # Ajustado para mejor visualización de curvas
...
def calculate_inverse_time_curve(tds, pickup, i_range):
    times = []
    if pickup <= 1e-6: return np.full_like(i_range, np.nan)
    for i in i_range:
        multiple = i / pickup
        if multiple <= 1.0: time = np.inf
        else:
            try:
                denominator = (multiple ** CURVE_B) - 1
                if denominator <= 1e-9: time = np.inf
                else: time = tds * (CURVE_A / denominator)
                if time < 0: time = np.inf
            except (OverflowError, ValueError): time = np.inf
        times.append(time)
    return np.nan_to_num(np.array(times), nan=np.nan, posinf=np.nan, neginf=np.nan)

# --- Fase 1: Análisis de Datos (Lógica EXACTA de tu primer script) ---
# (Código idéntico al anterior para esta fase: Carga, Filtro, Cálculo delta_t/mt, Clasificación)
coordinated_pairs = []; uncoordinated_pairs = []; tmt_total_scenario = 0.0
total_valid_pairs_scenario = 0; scenario_pairs_found = 0; skipped_pairs_count = 0; total_pairs_read = 0
try:
    print("Cargando datos (lista de pares)...")
    if not os.path.exists(input_file): raise FileNotFoundError(f"El archivo no existe: {input_file}")
    with open(input_file, 'r') as f: relay_pairs_data = json.load(f)
    print("Datos cargados correctamente.")
    if not isinstance(relay_pairs_data, list): raise TypeError(f"Error: {input_file} no contiene lista JSON.")
    print(f"Calculando delta_t, mt y clasificando pares para '{TARGET_SCENARIO_ID}'...")
    for pair_entry in relay_pairs_data:
        total_pairs_read += 1
        if not isinstance(pair_entry, dict): continue
        current_scenario_id = pair_entry.get("scenario_id")
        if current_scenario_id != TARGET_SCENARIO_ID: continue
        ...
        main_time = main_relay_info.get('Time_out'); backup_time = backup_relay_info.get('Time_out')
        if not isinstance(main_time, (int, float)) or not isinstance(backup_time, (int, float)):
            ...
            skipped_pairs_count += 1; continue
        delta_t = backup_time - main_time - CTI; mt = (delta_t - abs(delta_t)) / 2
        pair_info = copy.deepcopy(pair_entry); pair_info['delta_t'] = delta_t; pair_info['mt'] = mt
        if delta_t >= 0: coordinated_pairs.append(pair_info)
        else: uncoordinated_pairs.append(pair_info)
    ...
    if scenario_pairs_found == 0:
         ...
         total_valid_pairs_scenario = 0; miscoordination_count_scenario = 0; tmt_total_scenario = 0.0
    else:
    ...
    if skipped_pairs_count > 0: print(f"Pares omitidos DENTRO de '{TARGET_SCENARIO_ID}': {skipped_pairs_count}")
...
except FileNotFoundError: print(f"Error CRÍTICO: No se pudo encontrar: {input_file}"); exit()
except TypeError as e: print(f"Error CRÍTICO: Tipo de datos JSON inválido: {e}"); exit()
except json.JSONDecodeError as e: print(f"Error CRÍTICO: JSON mal formado: {e}"); exit()
except Exception as e: print(f"Error inesperado durante el procesamiento: {e}"); print("--- Traceback ---"); traceback.print_exc(); print("-----------------"); print("ADVERTENCIA: Intentando continuar...")

# --- Fase 2: Preparación de Datos para Dash (Sin cambios) ---
# (Código idéntico al anterior para esta fase: Dropdowns, Formato Tablas Resumen)
...
summary_columns_map = {
    "Falla (%)": "fault", "Línea Principal": "main_relay.line", "Relé Principal": "main_relay.relay", "TDS (Main)": "main_relay.TDS", "Pickup (Main)": "main_relay.pick_up", "I_shc (Main)": "main_relay.Ishc", "t_m (Main)": "main_relay.Time_out",
    "Línea Backup": "backup_relay.line", "Relé Backup": "backup_relay.relay", "TDS (Backup)": "backup_relay.TDS", "Pickup (Backup)": "backup_relay.pick_up", "I_shc (Backup)": "backup_relay.Ishc", "t_b (Backup)": "backup_relay.Time_out",
    "Δt": "delta_t", "MT": "mt"
}
def get_nested_value(d, key_path, default='N/A'):
    keys = key_path.split('.'); val = d
    try:
        for key in keys: val = val[key]
        if isinstance(val, (int, float)):
            if 'TDS' in key_path or 'Pickup' in key_path: return f"{val:.5f}"
            elif any(k in key_path for k in ['I_shc', 'Time_out', 'delta_t', 'mt']): return f"{val:.3f}"
            else: return val
        return val
...
# --- Fase 3: Creación de la Aplicación Dash (Sin cambios en layout) ---
# (Código idéntico al anterior para esta fase: Layout con Tabs, Dropdowns, Graphs, Tables)
...
def update_dashboard(coordinated_idx, uncoordinated_idx):
    ...
    error_messages = [] # Para acumular mensajes de datos faltantes

    # --- Procesar Par Coordinado ---
    if coordinated_idx is not None and coordinated_pairs:
        try:
            ...
            # Extraer datos usando .get() para evitar KeyError inicial
            main_tds = main_relay.get('TDS'); main_pickup = main_relay.get('pick_up'); main_ishc = main_relay.get('Ishc'); main_time_out = main_relay.get('Time_out'); main_name = main_relay.get('relay', 'N/A'); main_line = main_relay.get('line', 'N/A')
            backup_tds = backup_relay.get('TDS'); backup_pickup = backup_relay.get('pick_up'); backup_ishc = backup_relay.get('Ishc'); backup_time_out = backup_relay.get('Time_out'); backup_name = backup_relay.get('relay', 'N/A'); backup_line = backup_relay.get('line', 'N/A')
            fault_perc = pair.get('fault', 'N/A'); pair_delta_t = pair.get('delta_t'); pair_mt = pair.get('mt')
            ...
            if can_plot_points: # Si podemos graficar al menos los puntos
                coordinated_fig = go.Figure()
                i_range = None # Inicializar i_range

                # Generar rango de corriente si alguna curva se puede graficar
                if can_plot_main_curve or can_plot_backup_curve:
                    # Calcular max_current basado en Ishc disponibles y pickup*mult si las curvas son posibles
                    currents_for_max = [c for c in [main_ishc, backup_ishc] if isinstance(c, (int, float))]
                    if can_plot_main_curve: currents_for_max.append(main_pickup * MAX_CURRENT_MULTIPLIER)
                    if can_plot_backup_curve: currents_for_max.append(backup_pickup * MAX_CURRENT_MULTIPLIER)
                    max_current_val = max(currents_for_max) if currents_for_max else 10 # Default a 10 si no hay corrientes válidas

                    # Calcular min_current basado en pickups si las curvas son posibles
                    ...
                    # Asegurar que el rango mínimo es menor que el máximo antes de crear logspace
                    if i_range_min_val < max_current_val:
                        ...
                        i_range = None # No generar rango si min >= max


                # Graficar Curva Principal si es posible
                ...
                # Graficar Curva Backup si es posible
                ...
                # Graficar Puntos de Operación y líneas auxiliares (ya que can_plot_points es True)
                ...
                if i_range is not None: # Si se calcularon curvas, usar su rango
                     xaxis_config['range'] = [np.log10(i_range_min_val*0.9), np.log10(max_current_val*1.1)]
                else: # Si solo hay puntos, ajustar el rango a los puntos
            ...
            else: # No se pueden graficar ni los puntos
            ...
            # Preparar tabla de detalles independientemente de los errores de gráfico
            coordinated_table_data = [ {"parameter": "Escenario", "value": TARGET_SCENARIO_ID}, {"parameter": "Falla (%)", "value": fault_perc}, {"parameter": "Relé Principal", "value": f"{main_name} (Línea: {main_line})"}, {"parameter": "TDS (Main)", "value": f"{main_tds:.5f}" if isinstance(main_tds, (int, float)) else 'N/A'}, {"parameter": "Pickup (Main)", "value": f"{main_pickup:.5f} A" if isinstance(main_pickup, (int, float)) else 'N/A'}, {"parameter": "I_shc (Main)", "value": f"{main_ishc:.3f} A" if isinstance(main_ishc, (int, float)) else 'N/A'}, {"parameter": "t_m (Op. Main)", "value": f"{main_time_out:.3f} s" if isinstance(main_time_out, (int, float)) else 'N/A'}, {"parameter": "Relé Backup", "value": f"{backup_name} (Línea: {backup_line})"}, {"parameter": "TDS (Backup)", "value": f"{backup_tds:.5f}" if isinstance(backup_tds, (int, float)) else 'N/A'}, {"parameter": "Pickup (Backup)", "value": f"{backup_pickup:.5f} A" if isinstance(backup_pickup, (int, float)) else 'N/A'}, {"parameter": "I_shc (Backup)", "value": f"{backup_ishc:.3f} A" if isinstance(backup_ishc, (int, float)) else 'N/A'}, {"parameter": "t_b (Op. Backup)", "value": f"{backup_time_out:.3f} s" if isinstance(backup_time_out, (int, float)) else 'N/A'}, {"parameter": "Δt (t_b - t_m - CTI)", "value": f"{pair_delta_t:.3f} s" if isinstance(pair_delta_t, (int, float)) else 'N/A'}, {"parameter": "MT (Penalización)", "value": f"{pair_mt:.3f} s" if isinstance(pair_mt, (int, float)) else 'N/A'} ]
            # Añadir mensajes de error a la tabla si existen
            if error_messages:
                coordinated_table_data.append({"parameter": "Advertencia Gráfico", "value": "; ".join(error_messages)})

        except Exception as e:
            print(f"Error EXCEPCIÓN al procesar par coordinado {coordinated_idx}: {e}"); traceback.print_exc()
    ...
    # --- Procesar Par Descoordinado (Aplicar misma lógica de validación detallada) ---
    if uncoordinated_idx is not None and uncoordinated_pairs:
        error_messages_uncoord = [] # Reiniciar lista de errores
        try:
            # (Lógica de extracción y validación idéntica a la de coordinados)
            pair = uncoordinated_pairs[uncoordinated_idx]
            main_relay = pair.get('main_relay', {}); backup_relay = pair.get('backup_relay', {})
            main_tds = main_relay.get('TDS'); main_pickup = main_relay.get('pick_up'); main_ishc = main_relay.get('Ishc'); main_time_out = main_relay.get('Time_out'); main_name = main_relay.get('relay', 'N/A'); main_line = main_relay.get('line', 'N/A')
            backup_tds = backup_relay.get('TDS'); backup_pickup = backup_relay.get('pick_up'); backup_ishc = backup_relay.get('Ishc'); backup_time_out = backup_relay.get('Time_out'); backup_name = backup_relay.get('relay', 'N/A'); backup_line = backup_relay.get('line', 'N/A')
            fault_perc = pair.get('fault', 'N/A'); pair_delta_t = pair.get('delta_t'); pair_mt = pair.get('mt')
            ...
            if can_plot_points:
                ...
                # Graficar Puntos y líneas auxiliares
                ...
                xaxis_config = {'title': "Corriente (A)", 'type': "log"}
                yaxis_config = {'title': "Tiempo (s)", 'type': "log", 'range': [np.log10(MIN_TIME_PLOT), np.log10(MAX_TIME_PLOT)]}
                if i_range is not None: xaxis_config['range'] = [np.log10(i_range_min_val*0.9), np.log10(max_current_val*1.1)]
                else: xaxis_config['range'] = [np.log10(min(main_ishc, backup_ishc)*0.8), np.log10(max(main_ishc, backup_ishc)*1.2)]
            ...
            uncoordinated_table_data = [ {"parameter": "Escenario", "value": TARGET_SCENARIO_ID}, {"parameter": "Falla (%)", "value": fault_perc}, {"parameter": "Relé Principal", "value": f"{main_name} (Línea: {main_line})"}, {"parameter": "TDS (Main)", "value": f"{main_tds:.5f}" if isinstance(main_tds, (int, float)) else 'N/A'}, {"parameter": "Pickup (Main)", "value": f"{main_pickup:.5f} A" if isinstance(main_pickup, (int, float)) else 'N/A'}, {"parameter": "I_shc (Main)", "value": f"{main_ishc:.3f} A" if isinstance(main_ishc, (int, float)) else 'N/A'}, {"parameter": "t_m (Op. Main)", "value": f"{main_time_out:.3f} s" if isinstance(main_time_out, (int, float)) else 'N/A'}, {"parameter": "Relé Backup", "value": f"{backup_name} (Línea: {backup_line})"}, {"parameter": "TDS (Backup)", "value": f"{backup_tds:.5f}" if isinstance(backup_tds, (int, float)) else 'N/A'}, {"parameter": "Pickup (Backup)", "value": f"{backup_pickup:.5f} A" if isinstance(backup_pickup, (int, float)) else 'N/A'}, {"parameter": "I_shc (Backup)", "value": f"{backup_ishc:.3f} A" if isinstance(backup_ishc, (int, float)) else 'N/A'}, {"parameter": "t_b (Op. Backup)", "value": f"{backup_time_out:.3f} s" if isinstance(backup_time_out, (int, float)) else 'N/A'}, {"parameter": "Δt (t_b - t_m - CTI)", "value": f"{pair_delta_t:.3f} s (<0!)" if isinstance(pair_delta_t, (int, float)) else 'N/A'}, {"parameter": "MT (Penalización)", "value": f"{pair_mt:.3f} s" if isinstance(pair_mt, (int, float)) else 'N/A'} ]
            if error_messages_uncoord:
                 uncoordinated_table_data.append({"parameter": "Advertencia Gráfico", "value": "; ".join(error_messages_uncoord)})

        except Exception as e:
            print(f"Error EXCEPCIÓN al procesar par descoordinado {uncoordinated_idx}: {e}"); traceback.print_exc()
    ...
    # --- Gráfico de MT para descoordinados (Sin cambios) ---
    if uncoordinated_pairs:
        try:
            # (Lógica idéntica a la anterior para generar gráfico MT)
            ...
            for pair in uncoordinated_pairs:
                 if isinstance(pair.get("mt"), (int, float)) and pair.get("mt") < 0:
                     main_relay_info = pair.get('main_relay', {}); backup_relay_info = pair.get('backup_relay', {})
            ...
            if mt_values:
                ...
                mt_values_sorted = [mt_values[i] for i in sorted_indices]; mt_labels_sorted = [mt_labels[i] for i in sorted_indices]
                mt_fig = go.Figure()
            ...
            else: mt_fig = go.Figure(layout={'title': {'text': f"No hay pares descoordinados con MT < 0 ({TARGET_SCENARIO_ID})", 'x': 0.5}})
        except Exception as e: print(f"Error generando gráfico MT: {e}"); traceback.print_exc(); mt_fig = go.Figure(layout={'title': {'text': "Error al generar gráfico MT", 'x': 0.5}})
    else: mt_fig = go.Figure(layout={'title': {'text': f"No hay pares descoordinados en {TARGET_SCENARIO_ID}", 'x': 0.5}})
